...
@c.page>
    ...
    <table style="border-collapse: collapse; width: 100%;">
        ...
        <tbody>
        <
        #list users as userEntity>
            <tr style="border-bottom: 1px solid #ddd;">
                <td style="padding: 10px;">$
                {userEntity.id! "null"}</td>
                <td style="padding: 10px;">
                    <
                    #if userEntity.getFirstImage()?exists && userEntity.getFirstImage().name?has_content>
                        <img src="/userEntity/image/$
                        {userEntity.getFirstImage().name}" alt="Изображение котла" style="max-width: 100%; margin-top: 10px;">
                    <
                    /#if>
                </td>
                <td style="padding: 10px;">$
                {userEntity.username! "null"}</td>
                <td style="padding: 10px;">$
                {userEntity.getEmail()! "null"}</td>
                <td style="padding: 10px;"><
                #list userEntity.roles as role>$
                {role}<
                #sep>, <
                /#list></td>
                <td style="padding: 10px;">$
                {userEntity.getGender()! "null"}</td>
                <td style="padding: 10px;">$
                {userEntity.getCity()! "null"}</td>
                <td style="padding: 10px;">$
                {userEntity.getLastName()! "null"}</td>
                <td style="padding: 10px;"><
                #list userEntity.workerRoles as workerRoles>$
                {workerRoles}<
                #sep>, <
                /#list></td>
                <td style="padding: 10px;">
                    <
                    #if userEntity.getDateOfBirth()?has_content>
                        $
                        {userEntity.getDateOfBirth()?string("dd.MM.yyyy")}
                    <
                    #else>
                        Нет данных
                    <
                ...
                <td style="padding: 10px;"><a href="/userEntity/$
                {userEntity.id}">Редактировать</a></td>
                <td style="padding: 10px;"><a href="/userEntity/delete/$
                {userEntity.id}" style="color: red;">Удалить</a></td>
            </tr>
        <
        /#list>
        </tbody>
    </table>
    <form action="/userEntity/addProductTOIVO" method="post" accept-charset="UTF-8" enctype="multipart/form-data"
    ...
    <form action="/userEntity/addProductSUARI" method="post" accept-charset="UTF-8" enctype="multipart/form-data"
          style="max-width: 600px; padding: 20px; border: 1px solid #ddd; border-radius: 5px; box-shadow: 0 0 10px rgba(0, 0, 0, 0.1); background-color: #fff; margin: 20px 0 0 20px;">
        <input type="hidden" name="$
        {_csrf.parameterName}" value="$
        {_csrf.token}"/>
    ...
    <form action="/userEntity/addProductSALMI" method="post" accept-charset="UTF-8" enctype="multipart/form-data"
          style="max-width: 600px; padding: 20px; border: 1px solid #ddd; border-radius: 5px; box-shadow: 0 0 10px rgba(0, 0, 0, 0.1); background-color: #fff; margin: 20px 0 0 20px;">
        <input type="hidden" name="$
        {_csrf.parameterName}" value="$
        {_csrf.token}"/>
    ...
    <form action="/userEntity/addProductAINOVA" method="post" accept-charset="UTF-8" enctype="multipart/form-data"
          style="max-width: 600px; padding: 20px; border: 1px solid #ddd; border-radius: 5px; box-shadow: 0 0 10px rgba(0, 0, 0, 0.1); background-color: #fff; margin: 20px 0 0 20px;">
        <input type="hidden" name="$
        {_csrf.parameterName}" value="$
        {_csrf.token}"/>
    ...
    <
        ...
        #list allAinova as ainova>
            <div style="max-width: 600px; padding: 20px; border: 1px solid #ddd; border-radius: 5px; box-shadow: 0 0 10px rgba(0, 0, 0, 0.1); background-color: #fff; margin: 20px 0 0 20px;">
                <p>Модель: $
                {ainova.модельAINOVA}</p>
                <p>Мощность: $
                {ainova.мощность}</p>
                <p>Напряжение и частота: $
                {ainova.напряжениеИЧастота}</p>
                <p>Количество ступеней мощности: $
                {ainova.напряжениеИЧастота}</p>
                <p>Номинальный ток автоматического выключателя: $
                {ainova.номинальныйТокАвтоматическогоВыключателя}</p>
                <p>Сечение токопроводящей жилы: $
                {ainova.сечениеТокопроводящейЖилы}</p>
                <p>Wi-Fi: $
                {ainova.wiFi}</p>
                <p>Коэффициент полезного действия, не менее: $
                {ainova.коэффициентПолезногоДействия}</p>
                <p>Способ управления: $
                {ainova.способУправления}</p>
                <p>Использование в системах «Тёплый пол»: $
                {ainova.использованиеВСистемахТеплыйПол}</p>
                <p>Функционал ГВС: $
                {ainova.функционалГВС}</p>
                <p>Встроенный воздухоотводчик: $
                {ainova.встроенныйВоздухоотводчик}</p>
                <p>Встроенный предохранительный клапан: $
                {ainova.встроенныйПредохранительныйКлапан}</p>
                <p>Встроенный циркуляционный насос: $
                {ainova.встроенныйЦиркуляционныйНасос}</p>
                <p>Расширительный бак: $
                {ainova.расширительныйБак}</p>
                <p>Настройка расписания: $
                {ainova.настройкаРасписания}</p>
                <p>Погодозависимое управление: $
                {ainova.погодозависимоеУправление}</p>
                <p>Возможность подключения оборудования: $
                {ainova.возможностьПодключенияОборудования}</p>
                <p>Рабочее давление теплоносителя: $
                {ainova.рабочееДавлениеТеплоносителя}</p>
                <p>Диапазон регулирования температуры теплоносителя: $
                {ainova.диапазонРегулированияТемпературыТеплоносителя}</p>
                <p>Диапазон регулирования температуры воды: $
                {ainova.диапазонРегулированияТемпературыВоды}</p>
                <p>Класс влагозащищенности: $
                {ainova.классВлагозащищенности}</p>
                <p>Контур отопления подающая линия: $
                {ainova.контурОтопленияПодающаяЛиния}</p>
                <p>Контур отопления обратная линия: $
                {ainova.контурОтопленияОбратнаяЛиния}</p>
                <p>Масса аппарата, брутто: $
                {ainova.масса}</p>
                <p>Габаритные размеры (высота, ширина, глубина): $
                {ainova.габаритныеРазмеры}</p>
            </div>
        <
        /#list>
    <
        ...
        #list allSalmi as salmi>
            <div style="max-width: 600px; padding: 20px; border: 1px solid #ddd; border-radius: 5px; box-shadow: 0 0 10px rgba(0, 0, 0, 0.1); background-color: #fff; margin: 20px 0 0 20px;">
                <
                    ...
                    <img src="/userEntity/image/$
                    {salmi.getFirstImage().name}" alt="Изображение котла" style="max-width: 100%; margin-top: 10px;">
                <
                /#if>
                <p>Модель: $
                {salmi.модельSALMI}</p>
                <p>Объем: $
                {salmi.объем}</p>
                <p>Подключение к сети водоснабжения: $
                {salmi.подключениеКСетиВодоснабжения}</p>
                <p>Мощность: $
                {salmi.напряжениеИЧастота}</p>
                <p>Напряжение и частота: $
                {salmi.напряжениеИЧастота}</p>
                <p>Сила тока: $
                {salmi.силаТока}</p>
                <p>Рабочее давление теплоносителя: $
                {salmi.рабочееДавлениеТеплоносителя}</p>
                <p>Максимальная температура: $
                {salmi.максимальнаяТемпература}</p>
                <p>Термостат: $
                {salmi.термостат}</p>
                <p>Аварийный термодатчик: $
                {salmi.аварийныйТермодатчик}</p>
                <p>Уровень влагозащиты: $
                {salmi.уровеньВлагозащиты}</p>
                <p>Нагревательный элемент: $
                {salmi.нагревательныйЭлемент}</p>
                <p>Размер анода: $
                {salmi.размерАнода}</p>
                <p>Вход холодной воды: $
                {salmi.входХолоднойВоды}</p>
                <p>Выход горячей воды: $
                {salmi.выходГорячейВоды}</p>
                <p>Габаритные размеры (высота, ширина, глубина): $
                {salmi.габаритныеРазмеры}</p>
            </div>
        <
        /#list>
    <
        ...
        #list allSuari as suari>
            <div style="max-width: 600px; padding: 20px; border: 1px solid #ddd; border-radius: 5px; box-shadow: 0 0 10px rgba(0, 0, 0, 0.1); background-color: #fff; margin: 20px 0 0 20px;">
                <
                    ...
                    <img src="/userEntity/image/$
                    {suari.getFirstImage().name}" alt="Изображение котла" style="max-width: 100%; margin-top: 10px;">
                <
                /#if>
                <p>Модель: $
                {suari.модельSUARI}</p>
                <p>Тип камеры сгорания: $
                {suari.типКамерыСгорания}</p>
                <p>Модуляция пламени: $
                {suari.модуляцияПламени}</p>
        ...
        < #list allToivo as toivo>
            <div style="max-width: 600px; padding: 20px; border: 1px solid #ddd; border-radius: 5px; box-shadow: 0 0 10px rgba(0, 0, 0, 0.1); background-color: #fff; margin: 20px 0 0 20px;">
                < #if toivo.getFirstImage()?exists && toivo.getFirstImage().name?has_content>
                    <img src="/userEntity/image/$ {toivo.getFirstImage().name}" alt="Изображение котла" style="max-width: 100%; margin-top: 10px;">
                < /#if>
